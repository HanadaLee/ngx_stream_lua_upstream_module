...
ngx_stream_lua_upstream_get_upstreams(lua_State *L)
{
    ...
    if (umcf == NULL)
    {
        return luaL_error(L, "failed to get upstream main conf");
    }
...
ngx_stream_lua_upstream_find_upstream(lua_State *L, ngx_str_t *host)
{
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++)
    {
        ...
        if (uscf->host.len == host->len && ngx_memcmp(uscf->host.data, host->data, host->len) == 0)
        {
            return uscf;
        }
    }
    ...
    if (port)
    {
        ...
        for (i = 0; i < umcf->upstreams.nelts; i++)
        {
            ...
            if (uscf->port && uscf->port == n && uscf->host.len == len && ngx_memcmp(uscf->host.data, host->data, len) == 0)
            {
                return uscf;
            }
        }
