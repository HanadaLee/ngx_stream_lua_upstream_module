...
ngx_stream_lua_upstream_create_module(lua_State *L)
{
    ...
    lua_pushcfunction(L, ngx_stream_lua_upstream_current_upstream_name);
...
ngx_stream_lua_upstream_get_primary_peers(lua_State *L)
{
    ...
    peers = ngx_stream_lua_upstream_get_peers(L);
    if (peers == NULL) {
        return 2;
    }
    ...
    for (peer = peers->peer; peer; peer = peer->next) {
        ngx_stream_lua_get_peer(L, peer, i);
        lua_rawseti(L, -2, i + 1);
        i++;
    }
...
ngx_stream_lua_upstream_set_peer_down(lua_State * L)
{
    ...
    // maybe it should be a peers_rlock with peer_lock?
    ngx_stream_upstream_rr_peers_wlock(peers);

    peer = ngx_stream_lua_upstream_find_peer_by_id(L, peers, id);
    if (peer == NULL) {
        ngx_stream_upstream_rr_peers_unlock(peers);
        return 2;
    }
...
ngx_stream_lua_upstream_get_upstream_main_conf(lua_State *L)
{
    ...
    return ngx_stream_get_module_main_conf(r->session,
                                           ngx_stream_upstream_module);
}
...
static int
ngx_stream_lua_upstream_current_upstream_name(lua_State *L)
{
    ngx_stream_lua_request_t       *r;
